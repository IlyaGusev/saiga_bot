...
class EditImageTool(Tool):  # type: ignore
    name = "edit_image"
    description = """
    When there is an explicit command to edit an image or merge images,
    ...
    Do not return the path to the image, just directly the image itself.
    """
    inputs = {
        "prompt": {
            "type": "string",
            "description": "Prompt for the image editor in English",
        },
        "image_paths": {
            "type": "array",
    ...
    def forward(
        ...
        response = self.client.images.edit(
            model="gpt-image-1", prompt=prompt, image=[open(p, "rb") for p in image_paths]
        )
